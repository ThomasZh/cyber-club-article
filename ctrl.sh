...
#        ./crntb-adm.sh uninstall
        echo "Stopping article.7x24hs.com service, begin killing ${PID}"
    ...
    if [ "$PID" != "" ] ; then
        echo "Starting article.7x24hs.com service..."
        nohup python index.py -log_file_prefix=./log/article.log &
        echo $! > pidfile    #record process id to file
        echo 'Startup article.7x24hs.com service success!'
